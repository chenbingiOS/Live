...
+ (NSMutableAttributedString*)_attributedStringWithMessage:(EMMessage*)message
{
    ...
    if ([message.from isEqualToString:[EMClient sharedClient].currentUsername]) {
        ...
        UIColor *color = [UIColor colorWithRed:25/255 green:163/255 blue:255/255 alpha:1];
        [string addAttribute:NSForegroundColorAttributeName value:color range:NSMakeRange(range.length + range.location, text.length - (range.length + range.location))];
    }
    return string;
}
